...
namespace Main
{
    public abstract partial class Collection<T> : ICollection<T>
    {
        ...
        protected int AddCore(int index, T item, bool beforeAdd, bool afterAdd, bool throwError)
        {
            ...
            if (beforeAdd && !this.beforeAdd.Invoke(this, CollectionEventArgsType.BeforeAdd, index, item, out item)) return -1;
            SerializedCollection.Insert(index, item);
            if (afterAdd) this.afterAdd.Invoke(this, CollectionEventArgsType.AfterAdd, index, item, out _);
            return index;
        }
        protected int RemoveCore(int index, T item, bool beforeRemove, bool afterRemove, bool throwError)
        {
            ...
            if (beforeRemove && !this.beforeRemove.Invoke(this, CollectionEventArgsType.BeforeRemove, index, item, out _)) return -1;
            SerializedCollection.RemoveAt(index);
            if (afterRemove) this.afterRemove.Invoke(this, CollectionEventArgsType.AfterRemove, index, item, out _);
            return index;
        }
        //Add
        ...
        //Event
        public IObservable<CollectionBeforeAdd<T>> BeforeAdd => beforeAdd;
        public IObservableImmediately<CollectionAfterAdd<T>> AfterAdd => afterAdd;
        public IObservable<CollectionBeforeRemove<T>> BeforeRemove => beforeRemove;
        public IObservable<CollectionAfterRemove<T>> AfterRemove => afterRemove;
        ...
        public Collection(IEnumerable<T> collection = null)
        {
            beforeAdd = new();
