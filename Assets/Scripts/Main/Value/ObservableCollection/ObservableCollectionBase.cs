...
namespace Main.RXs
{
    public abstract partial class ObservableCollectionBase<T> : IObservableCollection<T>
    {
        ...
        protected int AddCore(int index, T item, bool beforeAdd, bool afterAdd, bool throwError)
        {
            ...
            if (beforeAdd && !this.beforeAdd.Invoke(this, ObservableCollectionEventArgsType.BeforeAdd, index, item, out item)) return -1;
            SerializedCollection.Insert(index, item);
            if (afterAdd) this.afterAdd.Invoke(this, ObservableCollectionEventArgsType.AfterAdd, index, item, out _);
            return index;
        }
        protected int RemoveCore(int index, T item, bool beforeRemove, bool afterRemove, bool throwError)
        {
            ...
            if (beforeRemove && !this.beforeRemove.Invoke(this, ObservableCollectionEventArgsType.BeforeRemove, index, item, out _)) return -1;
            SerializedCollection.RemoveAt(index);
            if (afterRemove) this.afterRemove.Invoke(this, ObservableCollectionEventArgsType.AfterRemove, index, item, out _);
            return index;
        }
        //Add
        ...
        //Event
        public IObservable<IObservableCollection_BeforeAdd<T>> BeforeAdd => beforeAdd;
        public IObservableImmediately<IObservableCollection_AfterAdd<T>> AfterAdd => afterAdd;
        public IObservable<IObservableCollection_BeforeRemove<T>> BeforeRemove => beforeRemove;
        public IObservable<IObservableCollection_AfterRemove<T>> AfterRemove => afterRemove;
        ...
        public ObservableCollectionBase(IEnumerable<T> collection = null)
        {
            beforeAdd = new();
